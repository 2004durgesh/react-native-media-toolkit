...
For more [examples](https://github.com/2004durgesh/react-native-video-toolkit/tree/main/example) check the example folder.

:::note
if using react-native-safe-area-context add `edges={['top']}` mainly in expo, else if you are using SafeAreaView from react-native no extra steps, this prevents the video content from being obscured by device notches or status bars.
:::
