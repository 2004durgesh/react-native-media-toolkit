...
The library provides a flexible theming system to easily customize colors, fonts, sizing, and other visual elements. Custom themes should be of type [`Theme`](./API%20Reference/Types/theme).
...
The `VideoPlayer` component accepts various props to configure its behavior. The `theme` prop is of type [`Theme`](./API%20Reference/Types/theme) and the `config` prop is of type [`VideoPlayerConfig`](./API%20Reference/Types/video#videoplayerconfig).
...
The `VideoPlayer` includes built-in gesture support for common interactions like double-tapping to seek and tapping to toggle controls. You can leverage more complex gestures by passing `gestureProps` to the `VideoPlayer` component. This prop accepts [`GestureHandlerProps`](./API%20Reference/Types/gestures#gesturehandlerprops).
